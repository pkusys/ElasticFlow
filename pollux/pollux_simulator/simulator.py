...
class Job(object):
    ...
    def __init__(self, name, application, submission_time,
                 target_num_replicas=None, target_batch_size=None, ddl=None, max_sample=None):
        self.name = name
    ...
    def step(self, seconds, interference=0.0):
        ...
        while seconds > 0 and self.completion_time is None:
            ...
            if self.max_sample:
                assert self.sample < self.max_sample
            ...
            if self.progress + goodput * seconds < next_progress:
                ...
                delta_iter = round(seconds / total_time)
                delta_sample = delta_iter * batch_size
                if self.max_sample and self.sample+delta_sample>=self.max_sample:
                    # sample>=max_sample
                    left_iter = math.ceil((self.max_sample-self.sample)/batch_size)
                    self.progress += goodput * left_iter
                    self.attained_service += left_iter*total_time*sum(self.placement)
                    self.sample += left_iter*batch_size
                    self.completion_time = self.current_time+left_iter*total_time
                    self.current_time += seconds
                    seconds = 0
                else:
                    # sample<max_sample
                ...
                delta_iter = round(delta/total_time)
                delta_sample = delta_iter * batch_size
                if self.max_sample and self.sample+delta_sample>=self.max_sample:
                    # sample>=max_sample
                    left_iter = math.ceil((self.max_sample-self.sample)/batch_size)
...
class Cluster(object):
    def __init__(self, workload, policy, min_nodes, num_gpus=4,
        ...
        if isinstance(policy, PolluxPolicy):
            if 'ddl' in workload.columns and 'max_sample' in workload.columns:
                self.jobs = [Job(row.name, APPLICATIONS[row.application], row.time, ddl=row.ddl,
                        max_sample=row.max_sample)
                         for row in workload.itertuples()]
            elif 'ddl' in workload.columns:
                self.jobs = [Job(row.name, APPLICATIONS[row.application], row.time, ddl=row.ddl)
                         for row in workload.itertuples()]
            elif 'max_sample' in workload.columns:
                self.jobs = [Job(row.name, APPLICATIONS[row.application], row.time, max_sample=row.max_sample)
                         for row in workload.itertuples()]
            else:
    ...
    def get_satisfactory_ratio(self):
        return {
            val["name"]: 1 if (val["ddl"] and val["completion_time"]<=val["ddl"]) else 0
            for val in self.logs[-1]["submitted_jobs"]
            if val["completion_time"] is not None
        }
    ...
    while not simulator.all_complete():
        ...
        for val in simulator.logs[-1]["submitted_jobs"]:
            if val["submission_time"] <= simulator.current_time and val["completion_time"] is None:
                print("    {}:\t[epoch {}]\t[restarts {}]\t[batch size {}]\t[placement {}]\t[sample {}/{}]".format(
                      val["name"], val["epoch"], val["num_restarts"], val["batch_size"], val["placement"], val["sample"], val["max_sample"]))
        used_gpus = sum(map(len, simulator.allocations.values()))
        print("GPU utilization: {}".format(used_gpus))
        print("Completed jobs:")
        jct_dict = simulator.get_jcts()
        # print(jct_dict)
        print(len(jct_dict.keys()))
