...
if __name__ == "__main__":
    ...
    templates = build_images(["gpt2", "bert", "resnet50", "vgg16", "inceptionv3", "deepspeech2"], args.repository)
    #templates = build_images(["deepspeech2"], args.repository)
    ...
    for row in workload.sort_values(by="time").itertuples():
        ...
        if not os.path.exists("/mnt/tensorboard/" + row.name):
            os.system("mkdir /mnt/tensorboard/" + row.name)
        ...
        env.append({"name": "TARGET_SAMPLES", "value": str(row.samples)})
        print(yaml.dump(job))
