...
class VGGWorkload():
    CHECKPOINT_TEMPLATE = '/mnt/data1/vgg_{}.pth'
    ...
    def __init__(self, job_id, master_addr, master_port, rank, local_rank, world_size, batch_size, iterations, gpu_id) -> None:
        ...
        self.checkpoint_path = VGGWorkload.CHECKPOINT_TEMPLATE.format(self.checkpoint_suffix)
        self.logger_prefix = f"[job {self.job_id} | VGG16]"
    ...
    def init_dataset(self):
        ...
        trainset = torchvision.datasets.ImageFolder(
            VGGWorkload.DATA_PATH,transforms.Compose([
    ...
    def checkpoint(self):
        ...
        if self.local_rank % 8 == 0: # save one copy on each node
            print(f"checkpoint the vgg16 model with acc {epoch_acc}%")
            self.best_acc = epoch_acc
            torch.save(self.ddp_model.module.state_dict(), VGGWorkload.CHECKPOINT_TEMPLATE.format(self.job_id))
