...
class ResNetCifar10Workload():
    CHECKPOINT_TEMPLATE = '{}.pth'
    DATA_PATH = "./"
    ...
    def __init__(self, job_id, master_addr, master_port, rank, local_rank, world_size, batch_size, iterations, gpu_id) -> None:
        ...
        self.checkpoint_path = ResNetCifar10Workload.CHECKPOINT_TEMPLATE.format(self.checkpoint_suffix)
    ...
    def init_dataset(self):
        ...
        trainset = torchvision.datasets.CIFAR10(root=ResNetCifar10Workload.DATA_PATH, 
    ...
    def checkpoint(self):
        ...
        if self.local_rank % 8 == 0: # save one copy on each node
            ...
            torch.save(self.ddp_model.module.state_dict(), ResNetCifar10Workload.CHECKPOINT_TEMPLATE.format(self.job_id))
