...
class InceptionV3Workload():
    CHECKPOINT_TEMPLATE = '/mnt/data1/inception_{}.pth'
    ...
    def __init__(self, job_id, master_addr, master_port, rank, local_rank, world_size, batch_size, iterations, gpu_id) -> None:
        ...
        self.checkpoint_path = InceptionV3Workload.CHECKPOINT_TEMPLATE.format(self.checkpoint_suffix)
        self.logger_prefix = f"[job {self.job_id} | Inception_v3]"
    ...
    def init_dataset(self):
        ...
        trainset = torchvision.datasets.ImageFolder(
            InceptionV3Workload.DATA_PATH,transforms.Compose([
    ...
    def load_from_checkpoint(cls, job_id, device):
        ...
        model = torchvision.models.inception_v3(init_weights=False, aux_logits=False).to(device)
    ...
    def checkpoint(self):
        ...
        #if self.best_acc < epoch_acc and self.local_rank & 8 != 0: # save one copy on each node
        if self.local_rank % 8 == 0: # save one copy on each node
            print(f"checkpoint the inceptionv3 model with acc {epoch_acc}%")
            self.best_acc = epoch_acc
            torch.save(self.ddp_model.module.state_dict(), InceptionV3Workload.CHECKPOINT_TEMPLATE.format(self.job_id))
