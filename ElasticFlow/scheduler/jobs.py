...
END: completed
ERROR
DECLINED: DDL requirement cannot be satisfied
'''
import numpy
import math
import utils
...
class _TFJobs(object):
    ...
    def __init__(self):
        ...
        self.declined_jobs = list() # declined/rejected
        ...
        self.num_declined_job = 0
        self.num_accepted_job = 0
    ...
    def add_job(self, job_dict):
        ...
        # for elastic scheduling, duration is unknown
        ...
        """if job_dict['job_idx'] % 10 < 9:
            job_dict['best_effort'] = 1
            job_dict['ddl'] = sys.maxsize
        else:
            job_dict['best_effort'] = 0"""
        if 'best_effort' in job_dict:
            if int(job_dict['best_effort']) == 1:
                job_dict['ddl'] = sys.maxsize
        ...
        job_dict['min_gpu'] = 1
        if FLAGS.schedule == 'ef' or FLAGS.schedule == 'ef-accessctrl':
            #or FLAGS.schedule == 'edf' or FLAGS.schedule == 'edf-accessctrl':
            job_dict['max_gpu'] = models.get_max_gpu(job_dict, FLAGS.gpu_type)
        else:
            job_dict['max_gpu'] = 0
            throughputs = THROUGHPUTS[job_dict['model']['name']][str(job_dict['batch_size'])]
            for num_gpu in throughputs:
                if throughputs[num_gpu] == "nan":
                    continue
                job_dict['max_gpu'] = max(job_dict['max_gpu'], int(num_gpu))
            job_dict['max_gpu'] = min(64, job_dict['max_gpu'])
            job_dict['max_gpu'] = min(job_dict['batch_size'], job_dict['max_gpu'])
    ...
    def run_all_jobs(self, timediff, cur_time):
        for job in self.runnable_jobs:
            if job['num_gpu'] == 0:
                continue
            if 'iter_left' not in job:
                job['iter_left'] = job['iteration']
            batch_size = str(job['batch_size'])
            run_time = timediff
            """if cur_time > job['end_time']:
                run_time -= (cur_time - job['end_time'])"""
            if 'real_end_time' in job:
                if cur_time > job['real_end_time']:
                    run_time -= (cur_time - job['real_end_time'])
            else:
                if cur_time > job['end_time']:
                    run_time -= (cur_time - job['end_time'])
            assert run_time > 0
            if run_time < job['overhead']:
                run_time = 0
            else:
                run_time -= job['overhead']
            #assert timediff >= job['overhead']
            iterations = run_time * float(THROUGHPUTS[job['model']['name']][batch_size][str(job['num_gpu'])])
            if job['iter_left'] <= iterations and job['end_time'] > cur_time:
                job['old_end_time'] = job['end_time']
                end_time = cur_time - timediff + math.ceil(job['iter_left'] / float(THROUGHPUTS[job['model']['name']][batch_size][str(job['num_gpu'])]))
                run_time = job['end_time'] - cur_time + timediff
                job['end_time'] = cur_time
                iterations = run_time * float(THROUGHPUTS[job['model']['name']][batch_size][str(job['num_gpu'])])
                self.change_job_end_event(job)
                job['end_time'] = end_time
            job['iter_left'] -= iterations
            job['overhead'] = 0
    ...
    def move_to_declined(self, job, remove_from_pending=False):
        job['status'] = 'DECLINED'
        self.declined_jobs.append(job)
        self.num_declined_job = len(self.declined_jobs)
    ...
    def add_job_end_event(self, job):
        ...
        if tmp_dict == None:
            ...
            tmp_dict['scale_jobs'] = list()
            tmp_dict['end_jobs'].append(job)
            self.job_events.append(tmp_dict)
        else:
            if job not in tmp_dict['end_jobs']:
                tmp_dict['end_jobs'].append(job)

        # ''' sort events based on their time'''
        # self.job_events.sort(key = lambda e:e.__getitem__('time'))

    def change_job_end_event(self, job):
        #for job end
        if job['old_end_time'] == job['end_time']:
            return
        tmp_dict = utils.search_dict_list(self.job_events, 'time', job['old_end_time'])
        if tmp_dict is not None:
            if job in tmp_dict['end_jobs']:
                tmp_dict['end_jobs'].remove(job)
            #if len(tmp_dict['end_jobs']) + len(tmp_dict['start_jobs']) + len(tmp_dict['scale_jobs']) == 0:
            if len(tmp_dict['end_jobs']) + len(tmp_dict['start_jobs']) == 0:
                self.job_events.remove(tmp_dict)
        del job['old_end_time']
        self.add_job_end_event(job)
            

    def add_job_scale_event_new(self, allocation_time):
        tmp_dict = utils.search_dict_list(self.job_events, 'time', allocation_time)
        if tmp_dict == None:
            #not found, add the time into to job_events
            tmp_dict = dict()
            tmp_dict['time'] = allocation_time
            tmp_dict['start_jobs'] = list()
            tmp_dict['end_jobs'] = list()
            self.job_events.append(tmp_dict)


    def add_job_scale_event(self, job, allocation_time):
        #for job end 
        if allocation_time not in job['allocations']:
            return
        tmp_dict = utils.search_dict_list(self.job_events, 'time', allocation_time)
        if tmp_dict == None:
            #not found, add the time into to job_events
            ...
            tmp_dict['end_jobs'] = list()
            tmp_dict['scale_jobs'] = list()
            tmp_dict['scale_jobs'].append(job)
            self.job_events.append(tmp_dict)
        else:
            tmp_dict['scale_jobs'].append(job)
            #print("new scaling event, ", tmp_dict)
    ...
    def add_migratable(self, job):
        ...
        1. distributed jobs # gdd: this is not necessary in ElasticScheduling
        2. running jobs
        3. ?
        '''
        #if job['num_w'] <= 1:
